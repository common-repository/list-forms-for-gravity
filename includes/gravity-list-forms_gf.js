...
// gravity-list-forms_gf.js
	
	window.addEventListener ('load', function() {
		console.log('list_forms_gravity GF Loaded.');
		...
		if (Table_Forms) {
			...
			for (i = 0; i < Table_Forms.children.length; i++) {
				...
				var Form_ID = Row.querySelector("[data-colname='ID']").children[0].innerHTML;
			...
			var WP_Ajax_Data = 'action=list_forms_gravity&mode=get_forms_description&forms=' + Forms + '&type=gf';
					...
					if (Notes) {	
						...
						for (i = 0; i < Notes.length; i++) {
							...
							// if (Note_Content) {
								...
								Note +=	'<span title="This is Form Description. To Edit - go to Form Settings.">';
								Note +=	Note_Label;
								Note +=	'</span>';
								Note +=	'<span>';
								Note +=	Note_Content;
